...
if __name__=='__main__':
    ...
    if True:
        ...
        recon.plot_img(np.abs(img_true_comb),picname='hw3/p2-fully-sampled-coilcombined-mag.png',savefig=True)
        recon.plot_img(np.angle(img_true_comb),picname='hw3/p2-fully-sampled-coilcombined-phase.png',savefig=True)


        # Question b: Undersampled images R=2
        # undersampling
        ny,nx,ncoils = kdata.shape
        sampling_matr = np.zeros((ny,nx))
        sampling_matr[::2,:] = 1
        ...
        img_R2 = recon.SENSE_recon(kdata_undersampled,coilmaps)


        # # Question d: R=4 SENSE reconed images
        # Compute the undersampled kspace datasampling_matr = np.zeros((ny,nx))
        sampling_matr = np.zeros((ny,nx))
        sampling_matr[::4,:] = 1
        kdata_undersampled = np.zeros_like(kdata)
        for coil_itr in range(ncoils):
        ...
        # # SENSE recon
        # img_R4 = recon.SENSE_recon(kdata_undersampled,coilmaps)
